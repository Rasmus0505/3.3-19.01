...
每完成一个任务后，必须在 `before` 创建的任务行为记录文件中填写实际操作行为。

1. 在任务池目录（`tasks/`）中查找对应的记录文件，文件名格式为 `Task{起始编号}-{结束编号}任务记录.md`。
2. 找到本次完成或修复任务对应的章节，补充以下关键内容：
   - 改动了哪些文件（精确到文件路径）
   - 修改了哪些函数或类
   - 每个改动点的关键逻辑或内容
   - 是否涉及数据库迁移
   - 实际执行的验证命令及结果
3. 格式不限，只需记录供后续审查 AI 判断任务是否正确完成的关键部分。
4. 若本次任务为 `fix` 或 `fixing`，还需说明修复了什么、修复前的问题是什么。
