...
- 在成功实现或修复后、正式回复用户前，必须主动复盘：本次改动是否改变了用户后续的启动、安装、打包、部署、配置、验证、更新或其他使用方式。
- 只要打包步骤、安装器流程、运行时捆绑策略、入口命令、验证命令、环境变量、迁移要求、重启/重建/重装要求、缓存失效条件、默认行为或首次运行流程发生变化，都要视为“用户感知行为差异”。
- 一旦确认或高度怀疑存在这类差异，不能只把它埋在普通总结里，必须放进最终回复里的专门告知区块；即使细节还不能 100% 确认，也要明确提醒用户。
...
- 任务池里是否仍然存在可执行工作。
- 一段结果导向的本地更新总结。
- 一个专门的“用户后续行为告知”区块，且固定使用以下三行独立输出：
  - `本次行为差异：...`
  - `生效条件：...`
  - `你需要做的事：...`
- 这三行用于明确告诉用户：本次是否改变了之后的操作方式、何时才会真正生效、以及用户接下来必须做什么。
- 如果本次没有行为差异，也必须显式写出 `本次行为差异：无`、`你需要做的事：无` 这类否定结论，不能省略。
- 如果差异只是高度怀疑、还没法 100% 证实，也要在这三行里直接写明 `可能`、`请确认`、`需要按以下方式验证` 等提示，而不是沉默跳过。
