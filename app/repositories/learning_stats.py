...
def list_learning_daily_stats(
    ...
    *,
    ...
    return list(
        db.scalars(
            select(UserLearningDailyStat)
            .where(
                UserLearningDailyStat.user_id == user_id,
                UserLearningDailyStat.stat_date >= start_date,
                UserLearningDailyStat.stat_date <= end_date,
            )
            .order_by(UserLearningDailyStat.stat_date.asc(), UserLearningDailyStat.id.asc())
        ).all()
    )
