"""repair lesson generation task schema location
...
def _create_task_table(schema: str | None) -> None:
    op.create_table(
        ...
        sa.Column("current_text", sa.String(length=255), nullable=False, server_default="等待处理中"),
...
def _ensure_indexes(schema: str | None) -> None:
...
def _ensure_columns(schema: str | None) -> None:
...
def upgrade() -> None:
    schema = _schema_name()
    if schema and not _has_table(TABLE_NAME, schema) and _has_table(TABLE_NAME, PUBLIC_SCHEMA):
        op.execute(f"ALTER TABLE {PUBLIC_SCHEMA}.{TABLE_NAME} SET SCHEMA {schema}")

    if not _has_table(TABLE_NAME, schema):
        _create_task_table(schema)

    _ensure_columns(schema)
    _ensure_indexes(schema)
