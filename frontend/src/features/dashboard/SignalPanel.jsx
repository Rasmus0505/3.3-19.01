...
export function SignalPanel({ report }) {
  return (
    <Card className="relative overflow-hidden rounded-[32px] border border-white/50 bg-white/78 shadow-[0_28px_80px_-44px_rgba(15,23,42,0.6)] backdrop-blur-xl">
      ...
      <CardContent className="relative p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500">Signal Deck</p>
            <h3 className="mt-2 text-2xl font-black tracking-tight text-slate-950">{report.stage.label}</h3>
            <p className="mt-2 text-sm leading-6 text-slate-600">{report.heroSummary}</p>
          </div>
          ...
          {report.signals.map((signal, index) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-[24px] border border-slate-200/80 bg-slate-50/90 p-4 shadow-[0_16px_40px_-36px_rgba(15,23,42,0.75)]"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <p className="text-lg font-black tracking-tight text-slate-950">{signal.value}</p>
                    </div>
                    <p className="mt-1 text-sm leading-6 text-slate-600">{signal.caption}</p>
                  </div>
                </div>
              </motion.div>
            );
